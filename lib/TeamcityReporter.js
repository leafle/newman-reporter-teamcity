...
class TeamcityReporter {
    ...
    beforeItem(err, args) {
        this.currItem = {name: this.itemName(args.item, args.cursor), passed: true, skipped: false, failedAssertions: [], ignoredAssertions: []};
    ...
    assertion(err, args) {
        if (err) {
            ...
            this.currItem.passed = false;
            this.currItem.skipped = true;
            this.currItem.ignoredAssertions.push(args.assertion);
        }
    }

    item(err, args) {
        if (this.currItem.skipped) {
            const msg = this.escape(this.currItem.ignoredAssertions.join(", "));
            console.log(`##teamcity[testIgnored name='${this.currItem.name}' message='${msg}' flowId='${this.flowId}']`);
        } else if (!this.currItem.passed) {
