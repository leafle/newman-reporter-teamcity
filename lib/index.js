...
class TeamcityReporter {
    ...
    beforeItem(err, args) {
        this.currItem = {name: args.item.name, passed: true, failedAssertions: [], folder: args.item.parent() };
        console.log(`##teamcity[testStarted name='${this.currItem.folder.name}/${this.currItem.name}' captureStandardOutput='true']`);
    }
    ...
    item(err, args) {
        if (!this.currItem.passed) {
            ...
            console.log(`##teamcity[testFailed name='${args.item.parent().name}/${args.item.name}' message='${msg}' details='${msg} - ${details}']`);
        }
        const duration = (this.currItem.response && this.currItem.response.responseTime) || 0;
        console.log(`##teamcity[testFinished name='${args.item.parent().name}/${args.item.name}' duration='${duration}']`);
    }
