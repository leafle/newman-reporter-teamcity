...
test('#item reports skipped tests correctly', () => {
  const emitter = new EventEmitter();
  const reporter = new TeamcityReporter(emitter, reporterOptions, options);
  ...
  emitter.emit('beforeItem', err, args2);
  emitter.emit('assertion', err, args2);
  emitter.emit('item', err, args2);

  expect(outputData).toEqual(expect.stringMatching(/~~~##teamcity\[testIgnored name='FolderName\/TestName' .*\]~~~/));
  const message = outputData.match(/~~~##teamcity\[testIgnored .* message='([^']*)' /);
  expect(message[1]).toEqual("Skipped test");
});